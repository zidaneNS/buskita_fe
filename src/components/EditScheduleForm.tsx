...
export default function EditScheduleForm({
    ...
    const defaultBus = buses.filter(bus => bus.name === schedule.bus?.name)[0];
    const defaultRoute = routes.filter(route => route.name === schedule.route?.name)[0];
    return (
        <form action={action} className="flex flex-col gap-y-3">
            ...
            <div className="flex flex-col gap-y-2">
                ...
                <select defaultValue={defaultBus.busId} name="bus_id" id="bus_id" className="px-4 py-2 rounded-md bg-white text-black">
            ...
            <div className="flex flex-col gap-y-2">
                ...
                <select defaultValue={defaultRoute.routeId} name="route_id" id="route_id" className="px-4 py-2 rounded-md bg-white text-black">
            ...
            <div className="flex gap-x-3 items-center py-4 w-fit px-2 border border-white/30 rounded-md">
                ...
                <input defaultChecked={schedule.isClosed} type="checkbox" name="closed" id="closed" />
            </div>
