    ...
    return (
        <div className="w-full flex flex-col gap-y-4">
            <form action={action} className="grid grid-cols-1 md:grid-cols-2 gap-x-2 gap-y-4 w-full min-h-full">
                ...
                <ProfileDetail isEditing={isEditing} type="text" name="nim_nip" setIsEditing={setIsEditing} attribute="NIM / NIP" value={user.nim_nip}>
                ...
                <ProfileDetail isEditing={isEditing} type="text" name="phone_number" setIsEditing={setIsEditing} attribute="Phone" value={user.phone_number}>
                    {state?.errors?.phone_number && <ErrorInputForm errMsg={state.errors.phone_number} />}
                </ProfileDetail>
                <ProfileDetail isEditing={isEditing} type="text" setIsEditing={setIsEditing} attribute="Credit Score" value={user.credit_score} />
                <ProfileDetail isEditing={isEditing} type="text" setIsEditing={setIsEditing} attribute="Role" value={user.role_name} />
